...
{
    public class RptMaxBuyerController : BaseController
    {
        [HttpPost]
        public virtual ActionResult GetSpShrRptMaxBuyer(string dateS, string dateE, string shrhKind,int? share, List<decimal> watchShareholderIds,string fullname)
        {
            var shrhCodeList = ShareholderWatchHelper.GetShrhCodeListFromWatchList(watchShareholderIds);

            var result = ReportHelper.GetSpShrMaxBuyerDashboard(dateS.Replace("/", ""), dateE.Replace("/", ""), shrhKind, share, shrhCodeList.Count != 0 ? shrhCodeList : null, StringHelper.ReplaceWithArabicChar(fullname), 0);
